...
if(isset($_POST['send']))
{
	$res=mysqli_query($con,"SELECT * FROM `admin_login` WHERE emp_id='$emp_id' AND username='$username'");
	if (mysqli_num_rows ($res) >0) 
    {	
		//if employee is true	
		$otp= rand(1000000,9999999);//randam otp generated
		$_SESSION['otp']=$otp;
		$res1=mysqli_query($con,"UPDATE admin_login SET otp = '$otp' WHERE emp_id = '$emp_id'");//otp send to database 
